...
public class Paillier
{
    ...
    private static Paillier instancia;
    ...
    private Paillier(int modLengthIn) throws Exception
    {
        if (modLengthIn < 8)
    ...
    public BigInteger sumar(BigInteger cifrado1, BigInteger cifrado2) {
        BigInteger product = cifrado1.multiply(cifrado2);
        BigInteger mod = product.mod(this.n.pow(2));
        return mod;
    }

    public BigInteger multiplicar(BigInteger cifrado, BigInteger plano) {
        return cifrado.modPow(plano, this.n.pow(2));
    }
}